  ...
  const handleAuth = async (e) => {
    ...
    try {
      if (view === 'login') {
        ...
        localStorage.setItem("edu_role", "student");
        navigate('/student');
      } else if (view === 'signup') {
        ...
        navigate('/student'); // Auto login after signup
      }
    } catch (err) {
      ...
      <div className="bg-white dark:bg-gray-800 p-12 rounded-[3rem] shadow-2xl w-full max-w-md animate-in slide-in-from-bottom-8 duration-500">
        ...
        <div className="text-center mb-8">
          ...
          <h2 className="text-3xl font-black text-gray-800 dark:text-white">
            ...
            {view === 'signup' && "Student Signup"}
            {view === 'forgot' && "Reset Password"}
          </h2>
          <p className="text-gray-500 dark:text-gray-300 font-medium">
            {view === 'forgot' ? "Answer security question to reset" : "Access your personalized AI tutor"}
          </p>
        </div>
          ...
          <form onSubmit={handleAuth} className="space-y-4">
            ...
            {view === 'signup' && (
              <div className="space-y-3 pt-2 border-t border-gray-100 dark:border-gray-700">
                ...
                <select value={securityQuestion} onChange={(e) => setSecurityQuestion(e.target.value)} className="w-full px-4 py-3 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-600 ">
        ...
        <div className="mt-6 flex flex-col gap-2 text-center text-sm">
          {view === 'login' && (
            <>
               <button onClick={() => setView('signup')} className="text-gray-500 hover:text-indigo-600 font-bold">New to EduAI? Create an account</button>
